...
from wtforms import TextAreaField , SelectMultipleField , IntegerField, StringField,PasswordField, SelectField, RadioField, BooleanField ,SubmitField,validators
...
def search_books():
    ...
    query = "SELECT DISTINCT b.ISBN, b.Title,b.Number_Of_Pages,b.Publisher,b.Language,b.Summary, sb.Available_Copies," \
    ...
    if author_first_name:
        query += " AND a.First_Name LIKE %s"
        params.append(f'%{author_first_name}%')

    if author_last_name:
        query += " AND a.Last_Name LIKE %s"
    ...
    return render_template('student_search_results.html', results=results)
    
@app.route('/rented_books')
def rented_books():
    username=session.get('username')
    query = "SELECT DISTINCT book.Title,book.ISBN,Rental.Rental_Date,Rental.Return_Date "\
            "FROM Rental "\
    ...
    return render_template('student_rented_books.html',rented_books=rented_books)

@app.route('/book/<isbn>')
def book_details(isbn):
    username = session.get('username')
    query = "SELECT DISTINCT b.ISBN, b.Title,b.Number_Of_Pages,b.Publisher,b.Language,b.Summary, sb.Available_Copies,b.Image_URL, " \
            "(SELECT GROUP_CONCAT(DISTINCT CONCAT(a.First_Name,' ',a.Last_Name) SEPARATOR ', ')" \
            "FROM Book_Author ba "\
            "INNER JOIN Author a ON a.Author_ID=ba.Author_ID "\
            "WHERE ba.ISBN=b.ISBN"\
            ...
            "INNER JOIN Book_Category bcat ON b.ISBN = bcat.ISBN " \
            "INNER JOIN Category c ON bcat.Category_ID = c.Category_ID " \
            "WHERE (u.Username = %s and b.ISBN=%s)"
    params=[username,isbn]
    cursor.execute(query,params)
    book_info=cursor.fetchone()
    if book_info:
        return render_template('book_details.html',book_info=book_info)
    else:
        return "Book not found"
    
@app.route('/reserve-user-book', methods=['POST'])
def reserve_book():
    try:
        ...
        if result:
            ...
            connection.commit()
            return "Book reserved successfully. <a href='" + url_for('student') + "'>Return to desired route</a>"
        else:
            return "Something went wrong"

    except Error as e:
        if e.errno == 1644:
            return "User has exceeded the weekly reservation limit. <a href='" + url_for('student') + "'>Return to Home Page</a>"
        else:

         return "An error occurred during reservation: " + str(e)
