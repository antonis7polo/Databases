    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        try:
            ...
            if error.errno == 1644:
                ...
                error_message = 'An error occurred while adding the school.'
...
def add_book():
    ...
    if form.validate_on_submit():
        try:
            ...
            image_file.save(os.path.join('static/images', filename))  # save the file
...
@app.route('/user/published_book_reviews')
def user_published_book_reviews():
    ...
    # Query to fetch all published book reviews
    reviews_query = """
        SELECT r.Published_Review_ID, r.ISBN, u.Username, r.Review_Text, r.Review_Date, r.Likert_Review
        FROM Published_Book_Review r
        INNER JOIN User u ON r.User_ID = u.User_ID
        WHERE u.School_Name = %s
        ORDER BY r.Review_Date DESC
    ...
    reviews = cursor.fetchall()
    message = request.args.get("message")
    return render_template('user_published_book_reviews.html', reviews=reviews, message = message)
...
def get_overdue_rentals():
    ...
    params = [first_name, last_name, delay_date, operator]
